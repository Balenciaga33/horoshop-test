...
test.describe('Checkout: валідація форми', () => {
  test('Порожній checkout — кнопка оформлення неактивна', async ({
    productPage,
    cartPage,
    checkoutPage,
  }) => {
  ...
  test('Неповний телефон — кнопка оформлення лишається неактивною', async ({
    productPage,
    cartPage,
    checkoutPage,
  }) => {
  ...
  for (const invalidEmail of validation.invalidEmails) {
    test(`Невалідний email "${invalidEmail}" — HTML5 помилка і замовлення не відправляється`, async ({
      productPage,
      cartPage,
      checkoutPage,
    }) => {
  ...
  test('Валідний email проходить HTML5-перевірку після невалідного', async ({
    productPage,
    cartPage,
    checkoutPage,
  }) => {
  ...
  test('Порожнє ПІБ при заповнених інших полях — submit може лишатися активним (поточна поведінка сайту)', async ({
    productPage,
    cartPage,
    checkoutPage,
  }) => {
