...
Мета test suite — стабільно ловити регресії критичних шляхів магазину Horoshop (UI + API), не перетворюючись на крихкий UI-snapshot фреймворк.
...
2. **Перевірка відсутності** там, де є ризик false-positive (товар, якого не має бути після фільтра)
...
Новий сценарій: будуємо знизу вгору — спочатку дивимось, чи достатньо API. UI додаємо лише коли треба перевірити те, чого API не покриває (розмітка, форма, клієнтська валідація, навігація).
